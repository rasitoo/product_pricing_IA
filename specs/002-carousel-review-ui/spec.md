...
- **FR-005**: El usuario DEBE poder rechazar la propuesta actual mediante swipe táctil hacia la izquierda o mediante un botón de rechazo visible. El rechazo directo (sin editar campos) resulta en `decision = reject` (sin `FeedbackSignal`). Si el operador abre el formulario inline de descripción/precio, solo se registra `decision = edit` (con `FeedbackSignal`) cuando al menos un campo tiene un valor distinto al original; si no hay cambio real, el resultado es igualmente `decision = reject`. El campo de motivo de rechazo es siempre opcional.
...
- **FR-013**: Cuando el operador confirma modificaciones sobre una propuesta (al menos un campo con valor distinto al original), el sistema DEBE enviar al backend las correcciones (campo modificado, valor original IA, valor corregido) como `FeedbackSignal`; la propuesta pasa a estado "modificada, pendiente de reaprobación" y vuelve al inicio de la cola. **Alcance v1**: los `FeedbackSignal` se almacenan en base de datos y se pasan como contexto few-shot al LLM en la siguiente propuesta del mismo tipo; el fine-tuning, RLHF y cualquier forma de reentrenamiento formal del modelo quedan fuera del alcance de esta versión.
